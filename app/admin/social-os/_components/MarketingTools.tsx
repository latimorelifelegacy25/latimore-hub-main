...
const CampaignAutoPilot = ({ onBulkSchedule }: { onBulkSchedule?: (posts: SocialPost[]) => void }) => {
  ...
  const generate = async () => {
    ...
    setScheduled(false);
    try {
      const res = await fetch('/api/admin/social-os/bulk-campaign', {
        method: 'POST',
...
const CanvaSpecGenerator = () => {
  ...
  const generate = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/admin/social-os/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: `Generate a Canva Creative Spec (copy/paste ready) for: "${goal}". Focus on visual hierarchy and high-impact educational copy for Central PA families.`, history: [] }),
      });
      const data = await res.json();
  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch('/api/admin/social-os/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: `Generate a complete Automation Asset Pack for: "${objective}". Include form fields, tagging strategy, and email nurture series copy that sounds like a helpful neighbor, not a robot.`, history: [] }),
      });
      const data = await res.json();
