...
export default function NotificationCenter({ className = '' }: NotificationCenterProps) {
  ...
  const [stats, setStats] = useState<NotificationStats>({ total: 0, unread: 0, byType: {}, byPriority: {} })
  ...
  const loadNotifications = useCallback(async () => {
    try {
      ...
      setStats(json.stats ?? { total: 0, unread: 0, byType: {}, byPriority: {} })
    } catch {
      // Notifications should never block admin navigation.
    }
  }, [filter])
          ...
          <div className="p-4 border-b border-slate-700">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                {stats.unread > 0 && (
                  <button onClick={handleMarkAllAsRead} className="text-xs text-blue-400 hover:text-blue-300">
                    Mark all read
                  </button>
                )}
              ...
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1 text-xs rounded ${
                    filter === key ? 'bg-[#C9A25F] text-slate-900' : 'text-slate-400 hover:text-white hover:bg-white/5'
                  }`}
                >
                  {label}
                </button>
