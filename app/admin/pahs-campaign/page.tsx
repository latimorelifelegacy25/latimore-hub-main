...
const INIT_METRICS: Metric[] = [
  { id:1, name:'Post Reach / Views',   baseline:274, target:1000, current:274, unit:'views',    kpi:'Brand Awareness',         tool:'Native Analytics' },
  { id:2, name:'QR Code Scans',        baseline:169, target:50,   current:169, unit:'scans',    kpi:'Physical-Digital Bridge',  tool:'Vercel Analytics' },
  { id:3, name:'Landing Page Visits',  baseline:0,   target:100,  current:0,   unit:'visits',   kpi:'Gateway Effectiveness',   tool:'GA4 G-S0Q3E4DEBJ' },
  { id:4, name:'Leads Captured',       baseline:0,   target:10,   current:0,   unit:'leads',    kpi:'Pipeline Fill Rate',       tool:'Supabase Dashboard' },
  { id:5, name:'Assessments Booked',   baseline:0,   target:5,    current:0,   unit:'bookings', kpi:'Monthly App Volume',       tool:'Google Calendar / CRM' },
  { id:6, name:'Referral Rate',        baseline:0,   target:25,   current:0,   unit:'%',        kpi:'Referral Growth',          tool:'Manual + Supabase' },
  { id:7, name:'District Contacts',    baseline:0,   target:2,    current:0,   unit:'meetings', kpi:'District Penetration',     tool:'CRM' },
]

const INIT_PIPELINE: PipelineStage[] = [
  { id:1, stage:'New Lead',             count:0, color:G,         desc:'Capture confirmed. Zapier triggers fired.',          kpi:'Did automation fire within 60s?' },
  { id:2, stage:'Contacted',            count:0, color:'#4A90C4', desc:'Manual DM or direct outreach initiated.',            kpi:'Was script delivered within timeline?' },
  { id:3, stage:'Assessment Scheduled', count:0, color:'#7B68EE', desc:'Lead used Google Calendar booking page.',            kpi:'Is the booking link loading on mobile?' },
  { id:4, stage:'Proposal Sent',        count:0, color:WA,        desc:'Tailored protection strategy delivered.',             kpi:'Was proposal personalized?' },
  { id:5, stage:'Closed',               count:0, color:GR,        desc:'Mission fulfilled. Secured legacy.',                 kpi:'Was referral ask made within 7 days?' },
]
    ...
    <div style={{ background:'#fff', borderBottom:'1px solid #e8e8e8', display:'flex', overflowX:'auto', gap:0 }}>
      {tabs.map(t => (
        <button key={t.id} onClick={() => setTab(t.id)}
          style={{ padding:'12px 18px', border:'none', borderBottom: tab===t.id ? `3px solid ${G}` : '3px solid transparent',
            background:'none', color: tab===t.id ? N : '#888', fontWeight: tab===t.id ? 700 : 400,
...
function StatCard({ metric, onUpdate }: { metric: Metric; onUpdate: (id: number, val: number) => void }) {
  ...
  return (
    <div style={{ background:'#fff', border:'1px solid #e8e8e8', borderRadius:10, padding:18, boxShadow:'0 2px 8px rgba(0,0,0,0.05)' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
        <div>
          <div style={{ fontWeight:700, color:N, fontSize:13, fontFamily:'Arial' }}>{metric.name}</div>
          <div style={{ fontSize:11, color:'#999', marginTop:2, fontFamily:'Arial' }}>{metric.tool}</div>
        </div>
...
function PipelineView({ pipeline, setPipeline }: { pipeline: PipelineStage[]; setPipeline: React.Dispatch<React.SetStateAction<PipelineStage[]>> }) {
  ...
  const updateCount = (id: number, val: number) =>
    setPipeline(prev => prev.map(s => s.id===id ? {...s, count:Math.max(0,val)} : s))
  ...
  return (
    <div>
      ...
      <div style={{ background:'#fff', border:'1px solid #e8e8e8', borderRadius:10, padding:20 }}>
        <div style={{ fontWeight:700, color:N, fontSize:14, marginBottom:16, fontFamily:'Arial', borderBottom:`2px solid ${G}`, paddingBottom:8 }}>Funnel Conversion Analysis</div>
        ...
        <div style={{ marginTop:16, padding:'12px 16px', background:LG, borderRadius:8 }}>
          <div style={{ fontSize:12, color:'#666', fontFamily:'Arial', lineHeight:1.7 }}>
            <strong style={{ color:N }}>Diagnostic Guide:</strong><br/>
            Reach high, scans low → Optimize physical placement &amp; QR visibility.<br/>
            Scans high, leads low → Improve landing page copy and form friction.<br/>
            Leads high, bookings low → Sharpen DM script and CTA clarity.<br/>
            Bookings high, closed low → Strengthen 15-min review presentation.
          </div>
        </div>
      </div>
    </div>
      ...
      <div style={{ background:'#fff', border:'1px solid #e8e8e8', borderRadius:10, padding:20 }}>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={history}>
            ...
            <Line type="monotone" dataKey="views" stroke={N} strokeWidth={2} dot={{ r:4 }} name="Views" />
        ...
        <div style={{ background:'#fff', border:'1px solid #e8e8e8', borderRadius:10, padding:20 }}>
          ...
          <ResponsiveContainer width="100%" height={140}>
            <BarChart data={history}>
              ...
              <Bar dataKey="bookings" fill={N} name="Bookings" radius={[3,3,0,0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
        ...
        <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
          ...
          <div style={{ display:'flex', alignItems:'flex-end' }}>
            <button onClick={addMonth}
              style={{ background:N, color:'#fff', border:'none', borderRadius:6, padding:'8px 18px', cursor:'pointer', fontWeight:700, fontSize:13, fontFamily:'Arial' }}>Add Month</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div>
      <div style={{ background:'#fff', border:'1px solid #e8e8e8', borderRadius:10, padding:20, marginBottom:16 }}>
        ...
        <div style={{ background:'#eee', borderRadius:6, height:10, overflow:'hidden' }}>
          <div style={{ width:`${(doneCount/allItems.length)*100}%`, background: doneCount===allItems.length ? GR : G, height:'100%', borderRadius:6, transition:'width 0.3s' }} />
        </div>
      ...
      {sections.map(([section, items]) => {
        ...
        return (
          <div key={section} style={{ background:'#fff', border:'1px solid #e8e8e8', borderRadius:10, padding:20, marginBottom:12 }}>
            <div style={{ fontWeight:700, color:N, fontSize:13, marginBottom:12, paddingBottom:8, borderBottom:`2px solid ${G}`, fontFamily:'Arial' }}>{section}</div>
            {items.map((item, i) => {
              const key = offset + i
              return (
...
function ComplianceView() {
  const items = [
    ...
    { area:'Visual Brand Lock',     status:'ACTION NEEDED', evidence:'Navy #2C3E50 and Gold #C49A6C not yet confirmed on all print asset files.',                              action:'Confirm exact hex values applied to all print assets before production print run.' },
  ]
  const actions = [
    'Integrate AED/cardiac survival narrative into PAHS content and game-day assets — dignity-first.',
    'Confirm all print assets use Navy #2C3E50 and Gold #C49A6C as exact hex values.',
      ...
      <div style={{ background:LG, border:`1px solid ${G}`, borderRadius:10, padding:20 }}>
        ...
        {actions.map((a, i) => (
          <div key={i} style={{ display:'flex', gap:10, padding:'8px 0', fontSize:13, color:'#333', fontFamily:'Arial', borderBottom: i < actions.length-1 ? '1px solid #e8d5bc' : 'none' }}>
            <span style={{ color:G, fontWeight:800, minWidth:20, fontFamily:'Arial' }}>{i+1}.</span>
            <span>{a}</span>
          </div>
        ))}
      </div>
...
// ── MAIN PAGE ─────────────────────────────────────────────────────────────────
export default function PAHSCampaignPage() {
  ...
  return (
    <div style={{ fontFamily:'Arial, sans-serif', background:'#f6f6f6', minHeight:'100vh', display:'flex', flexDirection:'column' }}>
      <div style={{ background:N, padding:'16px 24px' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:12 }}>
          <div>
            <div style={{ color:'#fff', fontWeight:800, fontSize:17, letterSpacing:0.5, fontFamily:'Arial' }}>LATIMORE LIFE &amp; LEGACY LLC</div>
            <div style={{ color:G, fontSize:12, marginTop:2, fontFamily:'Arial' }}>Latimore OS · PAHS Community Authority Framework · 2026</div>
          </div>
          <div style={{ display:'flex', gap:20 }}>
            <div style={{ textAlign:'right' }}>
              <div style={{ color:G, fontWeight:800, fontSize:24, fontFamily:'Arial' }}>{totalPct}%</div>
              <div style={{ color:'#aaa', fontSize:11, fontFamily:'Arial' }}>KPI Progress</div>
            </div>
            <div style={{ textAlign:'right' }}>
              <div style={{ color:'#fff', fontWeight:800, fontSize:24, fontFamily:'Arial' }}>{totalLeads}</div>
              <div style={{ color:'#aaa', fontSize:11, fontFamily:'Arial' }}>Pipeline Leads</div>
            </div>
          </div>
        </div>
        <div style={{ background:'rgba(255,255,255,0.15)', borderRadius:6, height:5, marginTop:12, overflow:'hidden' }}>
      ...
      <div style={{ background:N, color:G, textAlign:'center', padding:'10px 20px', fontSize:12, fontStyle:'italic', fontFamily:'Arial' }}>
        Protecting Today. Securing Tomorrow. — #TheBeatGoesOn · latimorelifelegacy.com
      </div>
    </div>
  )
}
