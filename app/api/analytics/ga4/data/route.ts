...
async function getAccessToken(refreshToken: string): Promise<string> {
  ...
  if (!res.ok || !data.access_token) {
    throw new Error(
      data.error_description || data.error || "Failed to refresh GA4 access token"
    );
  }
  ...
  try {
    ...
    if (!refreshToken) {
      return NextResponse.json(
        { error: "GA4 refresh token not found" },
        { status: 401 }
      );
    }
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Unexpected GA4 data error",
      },
      { status: 500 }
    );
  }
