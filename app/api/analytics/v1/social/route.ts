...
import { getSocialAnalytics, getDataQualityWarnings } from '@/lib/analytics/queries'
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [data, warnings] = await Promise.all([
      getSocialAnalytics(parsed.data),
    ...
    logger.error({ err }, 'analytics/v1/social error')
    return NextResponse.json({ ok: false, error: 'Failed to load social analytics.' }, { status: 500 })
  }
}
