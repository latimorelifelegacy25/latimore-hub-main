...
import { objectionHandlerPrompt } from '@/lib/ai/prompts'
...
export async function POST(req: NextRequest) {
  ...
  try {
    const aiRun = await createAiRun({
      ...
      input: { task: 'objection_handler', ...parsed.data },
    })
    aiRunId = aiRun.id

    const { system, user, schema, schemaName } = objectionHandlerPrompt(parsed.data)
