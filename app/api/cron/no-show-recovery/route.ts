...
export async function GET(req: NextRequest) {
  ...
  const events = await prisma.calendarEvent.findMany({
    where: { startAt: { lt: cutoff }, status: { in: ['scheduled', 'confirmed'] } },
  ...
  for (const event of events) {
    ...
    if (contact) {
      await prisma.task.create({
        data: {
          ...
          description: `Missed appointment on ${event.startAt.toLocaleString('en-US')}. Recovery email sent automatically; follow up by phone.`,
          dueAt: new Date(),
    ...
    await prisma.systemEvent.create({
      data: {
        ...
        payload: { eventId: event.id, scheduledFor: event.startAt.toISOString() },
      },
    })

    await sendGoogleChatMessage(
      `No-show detected\n\nContact: ${contactName}\nScheduled: ${event.startAt.toLocaleString('en-US')}\nRecovery email sent, follow-up task created.`,
    )
