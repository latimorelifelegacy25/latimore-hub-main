...
const CONTACT_REPLY = 'For personalized guidance, contact Jackson at 570-900-1977 or jackson1989@latimorelegacy.com. He can help you review options and next steps.'
...
function createLocalReply(message: string) {
  ...
  if (
    ...
    return [
      ...
      'You can call 570-900-1977 or email jackson1989@latimorelegacy.com to start.',
    ].join(' ')
  }
  ...
  if (normalized.includes('contact') || normalized.includes('phone') || normalized.includes('email') || normalized.includes('jackson')) {
    return 'You can reach Jackson M. Latimore Sr. by phone at 570-900-1977 or by email at jackson1989@latimorelegacy.com.'
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return jsonReply(
      reply || 'I can help with general insurance questions. For personalized guidance, please contact Jackson at 570-900-1977.',
    )
  } catch (error) {
    logger.error({ error }, '[chat] request failed')
    return jsonReply(
      'Sorry, chat is having trouble right now. Please call 570-900-1977 or email jackson1989@latimorelegacy.com.',
      { status: 500 },
    )
  }
}
